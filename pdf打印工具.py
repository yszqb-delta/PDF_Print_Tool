...
class RawPDF:
    ...
    def pdf_page_to_image(self, n, zoom=1):  # 将第n页转为图片
...
def run(raw_path):
    path = raw_path.split("/")[-1][:-4]+"(小册子).pdf"
...
button = tk.Button(root, text="运行", font=("Arial", 24), command=partial(run, file_path))
button.pack(pady=8)
